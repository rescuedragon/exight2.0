            ...
            <div className="p-2.5 pb-1.5 flex-shrink-0 relative z-10">
              <div className="flex items-center justify-between min-h-[1.25rem]">
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant="outline"
                    className={`${getExpenseColor(expense.type)} px-2.5 py-1 rounded-full font-medium text-xs backdrop-blur-md border-white/40 dark:border-gray-600/40 shadow-sm`}
                  >
                    {expense.type}
                  </Badge>
                  {isCompleted && (
                  ...
                  {!expense.isRecurring && !isCompleted && (
                    <DropdownMenu>
                            ...
                            <div className="space-y-6 mt-6">
                              <div className="p-6 backdrop-blur-md bg-gradient-to-br from-blue-accent/10 to-purple-accent/10 rounded-2xl border border-blue-accent/20 dark:border-blue-accent/30">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                  Remaining:{' '}
                                  {formatCurrency(
                                    selectedExpense?.remainingAmount || 0,
      ...
      return (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-muted-foreground text-lg mb-6 max-w-md">
            Start tracking your EMIs and recurring expenses to get insights into your financial
            commitments
          </p>
        </div>
      );
          ...
          <div className="backdrop-blur-xl bg-gradient-to-r from-white/10 via-white/5 to-white/10 dark:from-gray-900/20 dark:via-gray-800/10 dark:to-gray-900/20 border border-white/20 dark:border-gray-700/30 rounded-3xl overflow-hidden shadow-2xl">
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className="p-3 rounded-3xl shadow-xl backdrop-blur-sm bg-gradient-to-br from-amber-500/90 to-amber-600/90 hover:from-amber-500 to-amber-600 transition-all duration-300"
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400 tracking-wide">
                    {formatNumber(recurringExpenses.length)} active recurring payment
        ...
        {fixedTimeExpenses.length > 0 && (
          <div className="backdrop-blur-xl bg-gradient-to-r from-white/10 via-white/5 to-white/10 dark:from-gray-900/20 dark:via-gray-800/10 dark:to-gray-900/20 border border-white/20 dark:border-gray-700/30 rounded-3xl overflow-hidden shadow-2xl">
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className="p-3 rounded-3xl shadow-xl backdrop-blur-sm bg-gradient-to-br from-pink-500/90 to-pink-600/90 hover:from-pink-500 to-pink-600 transition-all duration-300"
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400 tracking-wide">
                    {formatNumber(fixedTimeExpenses.length)} active EMI
                    {fixedTimeExpenses.length !== 1 ? 's' : ''} & loan
                    {fixedTimeExpenses.length !== 1 ? 's' : ''}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-right">
                  <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 tracking-wide">
                    {formatCurrency(fixedTimeExpenses.reduce((sum, exp) => sum + exp.amount, 0))} /
                    month
                  </span>
                  <div className="text-xs text-muted-foreground">
                    Total due:{' '}
                    {formatCurrency(
                      fixedTimeExpenses.reduce((sum, exp) => sum + (exp.remainingAmount || 0), 0),
                    )}
                  </div>
                </div>
                {isFixedTimeExpanded ? (
