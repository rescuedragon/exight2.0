...
export const LoansDashboard = memo(({ loans, onUpdateLoan, isPrivacyMode = false }: LoansDashboardProps) => {
  ...
  const { activeLoans, completedLoans } = useMemo(() => {
    ...
    const completedLoans = loans.filter(loan => loan.status === 'completed' || loan.status === 'written-off');
  ...
  const handlePayment = useCallback((type: 'payment' | 'write-off') => {
    ...
    const newPayment: LoanPayment = {
      ...
      description: paymentDescription || `${type === 'payment' ? 'Payment' : 'Write-off'} of ${formatCurrency(payment)}`
    };
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className={`${getStatusColor(loan.status)} px-2.5 py-1 rounded-full font-medium text-xs backdrop-blur-md border-white/40 dark:border-gray-600/40 shadow-sm`}>
                <div className="flex items-center gap-1">
                  ...
                  {loan.status === 'active' ? 'Active' : loan.status === 'completed' ? 'Completed' : 'Written Off'}
                </div>
              </Badge>
              {loan.status === 'active' && (
                <DropdownMenu>
                      ...
                      <DialogContent className="fixed inset-0 z-[10000] w-full h-full bg-background backdrop-blur-xl bg-gradient-to-r from-white/20 via-white/10 to-white/20 dark:from-gray-900/30 dark:via-gray-800/20 dark:to-gray-900/30 border border-white/30 dark:border-gray-600/30">
                        ...
                        <div className="space-y-6 mt-6">
                          ...
                          <div className="flex justify-end space-x-3 pt-4">
                            ...
                            <Button variant="gradient" onClick={() => handlePayment('payment')} className="rounded-full px-6 backdrop-blur-sm bg-gradient-to-r from-blue-500/90 to-purple-500/90 hover:from-blue-500 to-purple-500">
                              Record Payment
                            </Button>
                          </div>
                        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-2 justify-items-center">
            ...
            <div className="flex items-center gap-1.5 justify-center w-full">
              <div className="text-center p-1.5 rounded-2xl bg-gradient-to-br from-white/20 to-white/10 dark:from-gray-800/30 dark:to-gray-700/20 backdrop-blur-sm border border-white/20 dark:border-gray-600/20 w-full">
                ...
                <p className="text-xs font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white bg-clip-text text-transparent">{formatCurrency(loan.totalReceived, loan.currency)}</p>
              </div>
            </div>
            
            <div className="flex items-center gap-1.5 justify-center w-full">
              <div className="text-center p-1.5 rounded-2xl bg-gradient-to-br from-white/20 to-white/10 dark:from-gray-800/30 dark:to-gray-700/20 backdrop-blur-sm border border-white/20 dark:border-gray-600/20 w-full">
                <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-0.5">Balance</p>
                <p className="text-xs font-bold bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white bg-clip-text text-transparent">{formatCurrency(loan.remainingAmount, loan.currency)}</p>
              </div>
            </div>
            
            <div className="flex items-center gap-1.5 justify-center w-full">
              <div className="text-center p-1.5 rounded-2xl bg-gradient-to-br from-white/20 to-white/10 dark:from-gray-800/30 dark:to-gray-700/20 backdrop-blur-sm border border-white/20 dark:border-gray-600/20 w-full">
                <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-0.5">Date</p>
  ...
  if (loans.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-muted-foreground text-lg mb-6 max-w-md">Start tracking your loans and money lending to get insights into your financial commitments</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {loans.length > activeLoans.length && (
        <div className="flex justify-end">
          <p className="text-sm text-muted-foreground">
            {loans.length - activeLoans.length} completed loan{loans.length - activeLoans.length !== 1 ? 's' : ''} in history
          </p>
        </div>
      )}
        ...
        <div className="backdrop-blur-xl bg-gradient-to-r from-white/10 via-white/5 to-white/10 dark:from-gray-900/20 dark:via-gray-800/10 dark:to-gray-900/20 border border-white/20 dark:border-gray-700/30 rounded-3xl overflow-hidden shadow-2xl">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-3xl shadow-xl backdrop-blur-sm bg-gradient-to-br from-blue-500/90 to-blue-600/90 hover:from-blue-500 to-blue-600 transition-all duration-300" style={{ backgroundColor: '#3b82f6' }}>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400 tracking-wide">{formatNumber(activeLoans.length)} active loan{activeLoans.length !== 1 ? 's' : ''}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  Received: {formatCurrency(activeLoans.reduce((sum, loan) => sum + loan.totalReceived, 0))}
                </div>
              </div>
              {isActiveExpanded ? (
      ...
      {completedLoans.length > 0 && (
        <div className="backdrop-blur-xl bg-gradient-to-r from-white/10 via-white/5 to-white/10 dark:from-gray-900/20 dark:via-gray-800/10 dark:to-gray-900/20 border border-white/20 dark:border-gray-700/30 rounded-3xl overflow-hidden shadow-2xl">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-3xl shadow-xl backdrop-blur-sm bg-gradient-to-br from-emerald-500/90 to-emerald-600/90 hover:from-emerald-500 to-emerald-600 transition-all duration-300" style={{ backgroundColor: '#10b981' }}>
                <CheckCircle className="h-5 w-5 text-white drop-shadow-lg" />
              </div>
              <div>
                <h3 className="text-xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-200 bg-clip-text text-transparent tracking-tight">Completed Loans</h3>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400 tracking-wide">{formatNumber(completedLoans.length)} completed loan{completedLoans.length !== 1 ? 's' : ''}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-right">
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-300 tracking-wide">
                  {formatCurrency(completedLoans.reduce((sum, loan) => sum + loan.amount, 0))} total
                </span>
                <div className="text-xs text-muted-foreground">
                  Received: {formatCurrency(completedLoans.reduce((sum, loan) => sum + loan.totalReceived, 0))}
                </div>
              </div>
              {isCompletedExpanded ? (
                <ChevronUp className="h-5 w-5 text-gray-600 dark:text-gray-400 transition-transform duration-300" />
